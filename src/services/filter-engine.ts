...
export function filterModelsList(list: ModelExtended[], filters: FilterEngineInputs | undefined): ModelExtended[] | null {
  ...
  if (filters) {
    ...
    if (filters.merges && filters.merges.length > 0) {
      processedList = processedList.filter((x) => arrayHasAll(filters.merges!, x.metadata.currentVersion?.merges ?? [], true));
    }
    ...
    if (filters.sort) {
      processedList.sort((a, b) => {
        ...
        else if (filters.sort!.key === 'merges')
